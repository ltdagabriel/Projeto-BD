  ...
  if($titulo == "" || $titulo == null){
    echo"<script language='javascript' type='text/javascript'>alert('O campo Titulo deve ser preenchido');window.location.href='index.php';</script>";
      ...
      if($obra == $titulo){
        ...
        $query2="INSERT INTO serie(stattus,obra_titulo,obra_data) value ('$status','$titulo','$data')";
        $insert = $connect->query($query);
        $insert2 = $connect->query($query2);
        
        if($insert){
            echo"<script language='javascript' type='text/javascript'>alert('Seu seriado foi adicionado!');window.location.href='index.php'</script>";
        }else{
          echo"<script language='javascript' type='text/javascript'>alert('Não foi possível cadastrar seu Seriado');window.location.href='index.php'</script>";
        }
      }
    }
?>
